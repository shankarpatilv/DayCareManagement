...
| P4-01 | Integrity Hardening | Enforce case-insensitive unique email                    | Copilot | Done        | Added migration `EnforceCaseInsensitiveEmailAndCleanupIndexes` with unique indexes on `lower(Email)` for Students/Teachers.                  |
| P4-02 | Integrity Hardening | Index cleanup and validation                             | Copilot | Done        | Removed redundant immunization unique index (composite PK already enforces uniqueness); migration applied and index checks passed.           |
| P5-01 | Quality Gate        | Testing and CI quality gate                              | Team    | Done        | Added deterministic resolver tests; CI now enforces EF model/migration consistency via `has-pending-model-changes`; local validation passed. |
| P6-01 | M3 Auth             | Auth/roles implementation (M3)                           | Team    | Done        | Implement authentication, authorization roles, and policy checks.                                                                            |
| P7-01 | Feature Migration   | Feature migration modules                                | Team    | In Progress | Completed first API slice; historical backfill regex blocker resolved (uppercase SHA-256 handling); strict `yyyy-MM-dd` route parsing completed; status remains In Progress because remaining migration modules are still pending. |
